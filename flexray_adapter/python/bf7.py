...
PROGRESS_FILE = os.path.expanduser("~/.flexray_adapter/bf7_progress.yaml")
...
class BFAlgo7:
  def __init__(self, config):
    ...
    self.gMacroPerCycle = self.config['gdStaticSlot'] * self.config['gNumberOfStaticSlots'] + adActionPointDifference + \
                     ...
                     self.config['gdNIT']
    self.values = BFAlgo7.generate_all_values()
...
def start():
  ...
  ex = BruteForceGUI(args, BFAlgo7, 'bf7.log', './test/audi_a4_b9.yml')
  sys.exit(app.exec())

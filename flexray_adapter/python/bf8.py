...
PROGRESS_FILE = os.path.expanduser("~/.flexray_adapter/bf8_progress.yaml")
...
class BFAlgo8:
  def __init__(self, config):
    ...
    self.gMacroPerCycle = self.config['gdStaticSlot'] * self.config['gNumberOfStaticSlots'] + adActionPointDifference + \
                     ...
                     self.config['gdNIT']
    self.values = BFAlgo8.generate_all_values()
    self.progress = load_progress()

  @staticmethod
  def generate_all_values():
    # [53, 53] gdStaticSlot
    # [3, 15] gdTSSTransmitter
    # [1, 18] gdActionPointOffset
    values = []
    for h in [53]:
      for i in [7, 7, 7, 7, 7]:
        for j in [7, 7, 7, 7, 7, 9, 9, 9, 9, 9]:
          values.append((h, i, j))
    return values
...
def start():
  ...
  ex = BruteForceGUI(args, BFAlgo8, 'bf8.log', './test/audi_a4_b9.yml')
  sys.exit(app.exec())

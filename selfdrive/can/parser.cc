...
// Expose can parser for controlsd.cc, so that it don't need to link to opendbc files.
extern "C" {
  void* canparser_init(int abus, const std::string& dbc_name,
            const std::vector<MessageParseOptions> &options,
            const std::vector<SignalParseOptions> &sigoptions) {
    CANParser *ret = new CANParser(abus, dbc_name, options, sigoptions);
    return (void*)ret;
  }

  void canparser_update_string(void *inst, std::string data) {
    CANParser *cp = (CANParser*)inst;
    cp->update_string(data);
  }

  void canparser_query_latest(void *inst, std::vector<SignalValue> &svs) {
    CANParser *cp = (CANParser*)inst;
    svs = cp->query_latest();
  }

  bool canparser_can_valid(void *inst) {
    CANParser *cp = (CANParser*)inst;
    return cp->can_valid;
  }
}

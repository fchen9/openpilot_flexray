...
from selfdrive.test.process_replay.compare_logs import remove_ignored_fields
...
def replay_process(cfg, lr, cmp_log_msgs):
  libcontrolsd_cc.enable_process_replay_test_mode()
  ...
  libcontrolsd_cc.process_replay_wait_for_can_sock_recv()

  ignore_fields = [k for k, v in cfg.ignore]

  ffi = FFI()
  log_msgs, msg_queue = [], []
  #for msg in tqdm(pub_msgs):
  cnt = 0
  for i in range(len(pub_msgs)):
    msg = pub_msgs[i]
    print('pub msg', i, msg.which(), 'recv events', len(log_msgs))
    recv_socks = [s for s in cfg.pub_sub[msg.which()] if
                    (libcontrolsd_cc.process_replay_cur_submaster_frame() + 1) % int(service_list[msg.which()].frequency / service_list[s].frequency) == 0]

    should_recv = bool(len(recv_socks))

    zmq_msg_data = ffi.new('unsigned char[]', msg.as_builder().to_bytes())
    if msg.which() == 'can':
      libcontrolsd_cc.process_replay_set_can_sock_data(zmq_msg_data, len(zmq_msg_data))
    else:
      libcontrolsd_cc.process_replay_set_sock_data(ffi.new('char[]', msg.which().encode('ascii')), zmq_msg_data, len(zmq_msg_data))

    if should_recv:
      libcontrolsd_cc.process_replay_update_msgs(0.0)

      recv_cnt = len(recv_socks)
      while recv_cnt > 0:
        out_buf_size = 65536
        out_buf = ffi.new('char[%s]' % out_buf_size)
        p_out_len = ffi.new('int[1]')
        libcontrolsd_cc.process_replay_wait_for_msg(out_buf, p_out_len, out_buf_size)
        m = log.Event.from_bytes(bytes(ffi.buffer(out_buf, p_out_len[0])))
        if m.which() != cmp_log_msgs[cnt].which():
          print(i, cnt, cmp_log_msgs[cnt].which(), m.which())
          return log_msgs
        msg1_bytes = remove_ignored_fields(m, cfg.ignore).as_builder().to_bytes()
        msg2_bytes = remove_ignored_fields(cmp_log_msgs[cnt], cfg.ignore).as_builder().to_bytes()

        if msg1_bytes != msg2_bytes:
          msg1_dict = m.to_dict(verbose=True)
          msg2_dict = cmp_log_msgs[cnt].to_dict(verbose=True)

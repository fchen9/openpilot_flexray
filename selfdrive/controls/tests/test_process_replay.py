...
from selfdrive.controls.tests.process_replay_controlsd_cc import replay_process, CONFIGS
...
if __name__ == "__main__":
  ...
  for segment in segments:
    ...
    # Cache this file for speeding up the development
    remove_file = False
    cached_rlog_fn = '/data/%s-%s-rlog.bz2' % (route_name.replace("|", "-"), segment_num)
    if not os.path.isfile(cached_rlog_fn):
      print('Downloading rlog for %s' % segment)
      rlog_fn = get_segment(segment)
      copyfile(rlog_fn, cached_rlog_fn)
      remove_file = True
    else:
      rlog_fn = cached_rlog_fn
    ...
    for cfg in CONFIGS:
      ...
      log_msgs = replay_process(cfg, lr, cmp_log_msgs)

      print(len(cmp_log_msgs), len(log_msgs))
      diff = []
      for i in range(len(cmp_log_msgs)):
        msg1 = cmp_log_msgs[i]
        msg2 = log_msgs[i]
        if msg1.which() != msg2.which():
          print("msgs not aligned between logs, %d, %s, %s" % (i, msg1.which(), msg2.which()))
          break
